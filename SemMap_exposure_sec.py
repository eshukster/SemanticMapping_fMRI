...
from psychopy import gui, data, core, event, logging, sound, visual
...
SemMap_filename = "SemMap_stim.csv"
...
for trial in trial_handler:
    ...
    while expClock.getTime()<trial['trial_start_time']:
        blue_cross.draw()
        win.flip()
        #core.wait(0.01)
    trial_time = expClock.getTime()
    
    #draw the image and word on the screen
    #draw the fixation
    #while expClock.getTime()<(trial['on_time']+trial['flip_time']):
    #while RT_resp.clock.getTime()<(trial['flip_time']):
    stim_img.draw()
    blue_cross.draw()
    win.flip()
    core.wait(trial['flip_time'])
    ...
    while RT_resp.clock.getTime()< (trial['display_time']-trial['flip_time']):
    ...
    # switch the fixation cross back to blue (basically for the ISI)
    blue_cross.draw()
    win.flip()
    
    while RT_resp.clock.getTime()< (trial['total_trial_time']):
        theseKeys = event.getKeys(keyList=['1', '2','3','4'])
        #if found:
            #RT_on_time='yes'
        if(len(theseKeys)>0 and not found):
    ...
    thisExp.addData('trial_start_time',trial['trial_start_time'])
